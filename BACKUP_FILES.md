...
Default upload settings are 256 MiB parts, eight transfers, and 128 MiB Drive upload chunks. Starting with 0.12.1, new uploads read the source once, computing whole-file SHA-256 and part SHA-256/MD5 while writing the bounded upload spool. Upload starts as soon as the first part is ready. Normal copy uploads have no separate source pre-scan or final source hash pass, and newly produced parts do not receive separate Python checksum scans. At most eight part files occupy the upload spool (2 GiB at defaults), with 1 GiB additional headroom. Each uploaded part is checked against Drive size/MD5 metadata before its staging copy is removed; file identity and timestamps are checked for changes. `--deep-verify` reads uploaded bytes back as well, including previously uploaded parts on retry. Explicit `--delete-local` moves retain a full source checksum check before deletion.

An interrupted single-pass upload rereads the source once to reconstruct its whole-file digest and validate the recorded part checksums, but does not re-upload verified cloud parts. Receipts checkpoint part checksums before sending each part. Unhashed entries are placeholders only in the local receipt; the final manifest and completion marker are published only after every part is hashed and verified. Cloud schema, catalog, and restore behavior are unchanged. Attempts started by older versions still resume through their original, more read-intensive verification path.
